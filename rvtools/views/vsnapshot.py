...
class dashModelForm(forms.ModelForm):
    class Meta:
        model = models.vSnapshot
        fields = '__all__'


def vsnapshot(request):
...
#   print(queryset)
    return render(request, 'vsnapshot.html', context)


def export_csv(request):
    # 定义要导出的表格名称
    table_name = "rvtools_vsnapshot"
    # 查询要导出的表格数据
    ...
    response["Content-Disposition"] = 'attachment; filename="snapshot-list.csv"'
